...
object TestPrintln extends App {
  override def run(args: List[String]): ZIO[zio.ZEnv, Nothing, ExitCode] = {
    ...
    def printSleepPrint(
      ...
      log(s"START: $name") *>
        URIO.sleep(sleep) *> log(s"DONE: $name") *>
        UIO(
          name
        )
    ...
    def printSleepFail(
      ...
      log(s"START: $name") *> URIO.sleep(sleep) *> log(s"FAIL: $name") *> IO
        .fail(new RuntimeException(s"FAIL: $name"))
    ...
    result
      .tapBoth(error => log(s"ERROR: $error"), v => log(s"WON: $v"))
